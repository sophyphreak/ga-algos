...
public class PatternMatching {
    ...
    public static List<Integer> boyerMoore(CharSequence pattern, CharSequence text, CharacterComparator comparator) {
        ...
        var last = buildLastTable(pattern);
        int i = 0;
        while (i <= text.length() - pattern.length()) {
            ...
            int shift = j > -1 ? last.getOrDefault(text.charAt(i + j), -1) : -1;
            if (shift < j) {
                i = i + j - shift;
            } else {
                i++;
    ...
    public static void main(String[] args) {
        var text = new String("aaaaaaaaa");
        var pattern = new String("aaa");
        var comp = new CharacterComparator();
        var result = boyerMoore(pattern, text, comp);
    }
}