...
public class AVL<T extends Comparable<? super T>> {
    ...
    private void updateHeightAndBF(AVLNode<T> currentNode) {
        ...
        int newBalanceFactor = leftHeight - rightHeight;
    ...
    private AVLNode<T> balance(AVLNode<T> currentNode) {
        ...
        if (currentNode.getBalanceFactor() == -2) { // right is heavy
            if (currentNode.getRight().getBalanceFactor() == 1) { // left-right rotation
                currentNode.setRight(rotateRight(currentNode.getRight()));
            }
            currentNode = rotateLeft(currentNode);
        } else if (currentNode.getBalanceFactor() == 2) { // left is heavy
            if (currentNode.getLeft().getBalanceFactor() == -1) { // left-rigth rotation
